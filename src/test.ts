...
import {torrentFile, torrentInfo} from "./types";
...
function verifyAllPropsValidTorrent(torrent: torrentInfo) {
...
function verifyAllPropsTypesMatchTorrentFile(torrentFile: torrentFile) {
    assert(typeof torrentFile.path === "string");
    assert(typeof torrentFile.length === "number");
}

function verifyAllPropsTypesMatchTorrent(torrent: torrentInfo) {
